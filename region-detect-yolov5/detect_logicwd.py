...
class UI_Logic_Window(QtWidgets.QMainWindow):
    ...
    def ruqin_flag(self):
        ...
        self.ruqin_check = self.ui.checkBox.isChecked()  #勾选后开启
        runqin_check_for_draw = self.ruqin_check

    # 打开json文件来获取到选择的区域，这个逻辑可以转移到web端，这里用的是QT的函数来实现文件打开读取
    def botton_area_open(self):
        self.openfile_area, _ = QFileDialog.getOpenFileName(self.ui.pushButton_2, '上传区域入侵json文件', 'ruqin/')
    ...
    # 与上一个函数配合使用，加载模型权重文件
    def open_model(self):
        self.openfile_name_model, _ = QFileDialog.getOpenFileName(self.ui.pushButton_weights, '选择weights文件', 'weights/')
    ...
    def model_init(self):
        ...
        if self.half:
            self.model.half()  # to FP16--CUDA是FP16
    ...
    def define_video(self):
        dict = {
            '钢筋加工厂': "http://hls01open.ys7.com/openlive/e5515428d3fc42***.hd.m3u8",
