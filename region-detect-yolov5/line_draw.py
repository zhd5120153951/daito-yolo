...
def draw_poly_area_dangerous(img, img_name, throughJSON=True):
    ...
    if throughJSON:
        ...
        cv2.polylines(img, [area_poly], isClosed=True, color=(0, 0, 255), thickness=3, lineType=cv2.LINE_AA)  #绘制多边形
    else:
        cv2.polylines(img, [img_name], isClosed=True, color=(0, 0, 255), thickness=3, lineType=cv2.LINE_AA)  #用全图作为多边形
...
def person_in_poly_area_dangerous(xyxy, area_poly):
    ...
    object_w = object_x2 - object_x1
    object_h = object_y2 - object_y1
    object_cx = object_x1 + (object_w / 2)  #预测物体框中心x
    object_cy = object_y1 + (object_h / 2)  #预测物体框中心y
