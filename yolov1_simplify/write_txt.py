...
VOC_CLASSES = (  # 定义所有的类名
    'hlb', 'blb', 'cp', 'td', 'dc', 'kqs', 'st', 'ylg')  # 使用其他训练集需要更改

# 定义一些参数
train_set = open('voctrain.txt', 'w')
test_set = open('voctest.txt', 'w')
# Annotations = 'VOCdevkit//VOC2007//Annotations//'
Annotations = 'E:\\Source\\Github\\datasets\\yolov1_simplify\\VOC2007\\Annotations\\'
...
def write_txt():
    ...
    for train_list in train_lists:  # 生成训练集txt
        ...
        for result in results:
            ...
            train_set.write(' ' + str(bbox[0]) + ' ' + str(bbox[1]) + ' ' + str(bbox[2]) + ' ' + str(bbox[3]) + ' ' +
                            str(class_name))
        train_set.write('\n')
    train_set.close()
    ...
    for test_list in test_lists:  # 生成测试集txt
        ...
        for result in results:
            ...
            bbox = result['bbox']
            class_name = VOC_CLASSES.index(class_name)
            test_set.write(' ' + str(bbox[0]) + ' ' + str(bbox[1]) + ' ' + str(bbox[2]) + ' ' + str(bbox[3]) + ' ' +
                           str(class_name))
        test_set.write('\n')
    test_set.close()
