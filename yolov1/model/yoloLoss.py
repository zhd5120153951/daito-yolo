...
class yoloLoss(nn.Module):
    def __init__(self, S, B, l_coord, l_noobj):
        ...
        self.S = S
        self.B = B
        self.l_coord = l_coord  # loss里面的的权重数，有目标l
        self.l_noobj = l_noobj  # 无目标l
    ...
    def forward(self, pred_tensor, target_tensor):
        ...
        coo_mask = target_tensor[:, :, :, 4] > 0  # 前四个class
        noo_mask = target_tensor[:, :, :, 4] == 0  # 前四个为零

        coo_mask = coo_mask.unsqueeze(-1).expand_as(target_tensor)
        noo_mask = noo_mask.unsqueeze(-1).expand_as(target_tensor)

        coo_pred = pred_tensor[coo_mask].view(-1, 30)  # batchsize*S*S, 30(class20,box1-5,box2-5)
        box_pred = coo_pred[:, :10].contiguous().view(-1, 5)  # box[x1,y1,w1,h1,c1]--box1、box2
        ...
        noo_pred_c = noo_pred[noo_pred_mask]  # noo_pred只需要计算 c 的损失 size[-1,2]
        ...
        return (self.l_coord * loc_loss + 2 * contain_loss + not_contain_loss + self.l_noobj * nooobj_loss +
                class_loss) / N
